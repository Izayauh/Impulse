...
try {
  # --- 0. Resolve the exact package under test ------------------------------
  $usingLocalArtifact = -not [string]::IsNullOrWhiteSpace($ArtifactDirectory)
  $assetRoot = $work

  if ($usingLocalArtifact) {
    $assetRoot = (Resolve-Path $ArtifactDirectory -ErrorAction Stop).Path
    $setupAsset = Get-ChildItem -Path $assetRoot -Filter 'Impulse-Setup-*.exe' -File |
      Sort-Object LastWriteTime -Descending | Select-Object -First 1
    if (-not $setupAsset) { throw "no Impulse-Setup-*.exe found in artifact directory: $assetRoot" }
    $tag = 'local PR artifact'
    $setup = $setupAsset.BaseName
    Step "Using local installer artifact: $($setupAsset.FullName)"
  } else {
    $rel = $null
    try {
  ...
  if (-not $usingLocalArtifact) {
    foreach ($name in $assetNames) {
      $dest = Join-Path $assetRoot $name
  ...
  $shaFile = Join-Path $assetRoot "$setup.sha256"
  if (Test-Path $shaFile) {
    $expected = ((Get-Content $shaFile) -split '\s+')[0].Trim().ToLower()
    $actual   = (Get-FileHash (Join-Path $assetRoot "$setup.exe") -Algorithm SHA256).Hash.ToLower()
  ...
  $p = Start-Process -FilePath (Join-Path $assetRoot "$setup.exe") `
