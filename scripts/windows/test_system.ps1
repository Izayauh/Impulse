...
foreach ($dll in $requiredDlls) {
    if (-not (Test-Path ("runtime\\bin\\" + $dll))) {
        $missingDlls += $dll
    }
}
...
if (Test-Path "runtime\\bin\\ggml-cuda.dll") {
    try {
...
$modelFiles = @(
    @{Path="runtime\\models\\ggml-base.en.bin"; Name="base.en"; Size=142},
    @{Path="runtime\\models\\ggml-medium.en.bin"; Name="medium.en"; Size=1500},
    @{Path="runtime\\models\\ggml-large-v3.bin"; Name="large-v3"; Size=3100}
)
$foundModels = @()
