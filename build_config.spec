...
APP_VERSION = '1.0.0'
MAIN_SCRIPT = 'flow_local_dictation.py'
...
for dll, dest in dll_files:
    dll_path = os.path.join(SPEC_DIR, dll)
...
for exe, dest in exe_files:
    exe_path = os.path.join(SPEC_DIR, exe)
...
datas = [
    ...
    (os.path.join(SPEC_DIR, 'models', 'ggml-base.en.bin'), 'models'),
    (os.path.join(SPEC_DIR, 'models', 'ggml-medium.en.bin'), 'models'),
    (os.path.join(SPEC_DIR, 'models', 'ggml-large-v3.bin'), 'models'),
    # Application icon
    (os.path.join(SPEC_DIR, 'Whisper.ico'), '.'),
]
